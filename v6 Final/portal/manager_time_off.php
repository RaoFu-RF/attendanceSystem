	...
	<?php include 'nav_bar.php' ?>
			...
			<div class="well col-lg-12">
				...
				<table id="table" class="table table-bordered table-striped">
					...
					<tbody>
					<?php
						$user_qry=$conn->query("SELECT * FROM emp_time_off") or die(mysqli_error());
							...
							<td>
								<?php
									if ($row['status'] == 'P') {
										?>
										<center>
											<button class="btn btn-sm btn-outline-primary accept_request" data-toggle="tooltip" data-placement="top" title="Accept Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-check"></i></button>
											<button class="btn btn-sm btn-outline-danger decline_request" data-toggle="tooltip" data-placement="top" title="Decline Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-times"></i></button>
											<button class="btn btn-sm btn-outline-danger remove_request" data-toggle="tooltip" data-placement="top" title="Remove Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-trash"></i></button>
										</center>
										<?php
									} elseif($row['status'] == 'A') { 
											?>
											<center>
												<button class="btn btn-sm btn-outline-primary edit_request" data-toggle="tooltip" data-placement="top" title="Edit Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-edit"></i></button>
												<button class="btn btn-sm btn-outline-danger decline_request" data-toggle="tooltip" data-placement="top" title="Decline Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-times"></i></button>
												<button class="btn btn-sm btn-outline-danger remove_request" data-toggle="tooltip" data-placement="top" title="Remove Request" data-id="<?php echo $row['request_id']?>" type="button"><i class="fa fa-trash"></i></button>
											</center> <?php 
										} elseif($row['status'] == 'D') { 
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$('.accept_request').click(function(){
				...
				var _conf = confirm("Are you sure you want to accept this request?");		

				if(_conf == true){
					$.ajax({
						url:'accept_request.php?request_id='+request_id,
			...
			$('.decline_request').click(function(){
				var request_id=$(this).attr('data-id');
				var _conf = confirm("Are you sure you want to decline this request?");		

				if(_conf == true){
					$.ajax({
						url:'decline_request.php?request_id='+request_id,
						error:err=>console.log(err),
						success:function(resp){
							if(typeof resp != undefined){
								resp = JSON.parse(resp)
